...
- **初始化**：
  - 从 releases 下载 e2echo-server-gateway ，解压
  - 修改解压后文件夹中的 application.yml 配置，需要配置 Nacos 服务地址
- **运行**：在jar文件所在目录中运行：`java -jar e2echo-server-gateway.jar` 即可
...
- **Redis**：5.0 或更高版本（服务端运行时必需）
- **RabbitMQ**：4.2 或更高版本（服务端运行时必需）
- **MinIO**：RELEASE.2025-04-22T22-12-26Z 或更高版本（服务端运行时必需）
- **Nacos**：2.5 或更高版本（服务端运行时必需）
- **Git**: 用于克隆代码库
